...
import { listPublicPeerReviews } from '../lib/peer-ratings.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return json(res, 200, {
      group: meta,
      ratings: rows,
      windowDays: Math.round(PUBLIC_PEER_REVIEW_WINDOW_MS / (24 * 60 * 60 * 1000)),
      maxPerPair: PUBLIC_PEER_REVIEW_MAX_PER_PAIR,
    });
  } catch (err) {
    return error(res, 500, getErrorMessage(err, 'Failed to load peer reviews'));
  }
}
